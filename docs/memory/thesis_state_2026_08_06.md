...
metadata: 
  ...
  modified: 2026-08-11T02:52:03.669Z
---
...
**Bill 0007 ENACTED 2026-08-09** (docs/governance/bills/0007, Case 4): ALL aggregate analyses use calibrated depth d_cal = d − d_first_success (per variant/calibration epoch; shallowest all-success zero-tilt cell = 0; tilt cells inherit their variant's zero) and the window d_cal ∈ [0, 10] mm ONLY (R4 2026-08-10 reverted R3's 12mm; abort-closure + rigid-spans-closed + pj y-symmetry rulings remain). R1: per-SERIES zeroing (each variant×condition zeroed at own first successful cell). R5 (2026-08-10): every rendered curve must span the full [0,10] window — endpoint at exactly d_cal=10 interpolated from the first out-of-window support cell, line-only (no marker); series with no support beyond their last cell (rigid jaws) end where data ends. R2: majority rule — a cell is successful when ≥2/3 of valid attempts succeed; curation-relabelled (outlier) trials and aborted rows are excluded from verdicts; safety-abort cells CLOSE ranges but never enter force stats. d_first_success values: pj_stock +1.5, pj_TPU +1.0, fr18 −1.5, fr18_model2(Jul21) 0.0, fr26 −0.5, fr26_5deg 0.0. Finray ranges are right-censored → claim "≥10 mm vs ≈0.5–2.5 mm rigid (closed)". Deprecated data excluded from counts (845 analysed trials): m2 pre-recalibration June cells, fr26 tx+5° pilots, superseded same-config sessions. Any figure/statistic regenerated after this date must follow this convention. COLLECTOR CAVEAT (2026-08-14): pick the newest VALID cell per depth, not the newest dir — the 2026-08-11 crashed fr18 sessions (all rows aborted) silently shadowed the valid June +5.0mm cell and shifted the m1-vs-m2 metrics until fixed.
