...
on a wedge inclined at $\beta = 5.0^{\circ}$, with the slope aligned
with the gripping direction. Each variant is then calibrated \emph{blind}
...
attempts, while the rigid jaw fails or is marginal: a $5^{\circ}$
surface-relative misalignment is twice the largest runtime tilt at which
the rigid jaws retained a usable window, and that window had already
narrowed to between $0.5$ and $2.5$\,mm under $\pm 2.5^{\circ}$ tilt.
