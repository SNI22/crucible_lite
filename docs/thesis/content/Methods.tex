...
used a $\alpha = +5.0^{\circ}$ seat in place of the default
...
toward each other rather than splaying apart, closing the residual slit
that would otherwise remain between the finger tips when the gripper is
fully closed, ensuring firm contact between the fingers along their
...
$\sigma_{z,\text{tcp}} \leq 102$\,\textmu m.
...
\begin{equation}
  ...
  \label{eq:dcal}
\end{equation}
where $d_{\text{first success}}$ is, per variant and calibration epoch, the
shallowest zero-tilt cell whose completed trials are all successful.
Tilt-condition cells inherit $d_{\text{first success}}$ from their
variant's zero-tilt sweep in the same calibration epoch. The reference is
...
$d_{\text{cal}} \in [0, 12]$\,mm; cells outside this window remain in the
record but are excluded from aggregates. This convention was adopted as Bill~0007 of the project's governance
record (enacted 2026-08-09; revised to per-series zeroing, a
majority-success cell criterion, and the 12\,mm window on 2026-08-10),
