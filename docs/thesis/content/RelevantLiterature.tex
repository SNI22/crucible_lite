...
from a CAD model. Early work in this space focused on perceiving that
...
manipulator to grasp --- an early demonstration that a physics-informed
shape prior, rather than raw image features, is what makes cloth state
...
directly from silhouettes robust to texture and colour variation, letting
...
to plan \emph{regrasp} actions that unfold and flatten a crumpled garment
...
singulated from a stack --- direct evidence that fingertip force/tactile
information, not vision alone, is necessary to resolve certain cloth-grasp
ambiguities. For a comprehensive treatment of the field beyond these
...
\TODO{Add a citation to recent McGill ADL-ROS lab work on cloth or
deformable-object manipulation if one exists -- a verified literature
search (2026-07-30) could not confirm a specific matching publication for
this lead. If the intended reference is unpublished/internal lab work,
cite it accordingly (e.g.\ technical report or personal communication per
McGill GPS guidelines) rather than as a peer-reviewed source.}
...
the pick. Ramisa et al.~\cite{ramisa2012wrinkled} detect and rank
\emph{wrinkles} in depth images of crumpled laundry, directing a standard
gripper to the most graspable bump on the cloth rather than to a flat
region. Qian et al.~\cite{qian2020clothregion} segment cloth \emph{edges
...
approaches --- intrusive (needle/pin) grippers that pierce the top ply,
and adhesive/electrostatic surface grippers --- developed largely for
garment manufacturing. Electroadhesion has since matured substantially;
...
grippers across payload, adaptability, and precision metrics --- but such
comparisons are conducted on rigid or quasi-rigid workpieces, not on
cloth, and report capability envelopes rather than parametric contact
behaviour. None of the work above addresses the specific question this
thesis investigates: almost all cloth-manipulation literature treats grasp
\emph{success} as a binary outcome to be predicted or achieved, not as a
function of a continuous physical parameter space (press depth, gripper
tilt) whose boundary can be mapped and compared across gripper geometries.
...
fixed, varies only the finger design $(t, \alpha)$, and maps the resulting
...
diameter --- the same three quantities (force, indentation depth, contact
geometry) that this thesis's depth-sweep experiments characterise directly
...
for this class of compliant finger. On the empirical side, Crooks et
...
thesis's use of two TPU Shore-hardness variants, Lang et
...
direct prior evidence that finger material hardness, independent of
...
On the Franka FR3 platform specifically, this impedance relationship is
realised through the Franka Control Interface's real-time Cartesian and
joint impedance controllers, documented and distributed as reference
implementations in libfranka~\cite{frankarobotics_libfranka}. The
Cartesian impedance controller applies a spring-damper law referenced to
a commanded pose with independently tunable translational and rotational
stiffness, run at the robot's real-time servo rate; the tilt-aware pose
targeting used in this thesis's trial procedure
(Section~\ref{sec:tilt-pose}) builds directly on this controller.

Contact force in this thesis is measured through two independent
channels: the Franka FCI's own external-wrench estimate, and (per the
project's toolchain configuration) an array of Tekscan FlexiForce
resistive force sensors. FlexiForce is a thin-film piezoresistive sensor
...
through franka\_ros2~\cite{frankarobotics2026franka_ros2}, the official
ROS\,2 integration built on \texttt{libfranka} and \texttt{ros2\_control},
rather than through libfranka directly, consistent with the toolchain
recorded in \texttt{docs/toolchain\_config.md}.
...
load-cell measurements during controlled contact --- the same FCI
external-wrench signal this thesis uses as one of its two Contact Force
channels, so their accuracy findings directly bound how much the FCI-based
channel alone can be trusted without the FlexiForce cross-check.
...
Franka Panda as one of its physical platforms.
