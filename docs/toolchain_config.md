...
|------|--------|---------|-------|
| ~/crucible/cloth-grasp (this repo) | `cloth_grasp` | Constitutional governance + analysis / scaffold infrastructure. Owns Amendment 1, Channel & Topic Map, Data Stream Format, and the per-trial post-processing once it lands in `src/`. Remote: `git@github.com:SNI22/crucible_lite.git`. | Commit policy here: any change to constitutional records (`docs/governance/`, `docs/device_context.md`, `docs/toolchain_config.md`) must reference a ratified Amendment or an enacted Bill in the commit message. Framework lives separately at `~/crucible/core/` (installed via pipx); see crucible-core README for the framework/instance split. |
